...
def main():
    ...
    # ランダムフォレスト学習結果出力するディレクトリを取得する。
    path = homeDir + 'machinelearn/sigmoid_RFT/'

    # ランダムフォレストクラスを取得する。
    RFTClass = RFT_class.RFT(pid)
    ...
    # RFTで予想
    clf = RFTClass.randomForestMikeModel_Class(X_train,Y_train,path + dir,n_estimators=350, random_state=0)
...
#########################################################################################################
    ...
    # 学習結果をロードする.
    clf2 = RFTClass.randomForestModelload(path + dir)
